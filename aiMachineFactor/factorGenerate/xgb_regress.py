...
from aiMachineFactor.dataPrepare.sample_handle import SubSample, feature_label
...
def rolling_generate():
    ...
    bench_ind = SubSample.bench_huge(indexdata)
    ...
    for ind, trade_date in resample_ind.iteritems():
        ...
        try:
            # 调用模型
            model = GridXgbRegress(out_end, g_params, common_params)
            model.fit(samplein_data, feature, label)
            model.predict(sampleout, feature, label)
            # 保存结果
...
class GridXgbRegress(object):
    ...
    def fit(self, samplein, feature, label, cv=5):
        ...
        # 训练
        model = xgb.XGBRegressor(**self.common_params)
    ...
    def predict(self, sampleout, feature, label):
        ...
        # 训练
        result = self.gsearch.predict(x_test)
        temp = sampleout.copy()
        temp['pre'] = result.T

        self.pre_df = temp[['date', 'windcode']+label+['pre']]
        evs = metrics.explained_variance_score(y_test, result)
